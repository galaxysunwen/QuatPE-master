...
class QuatPE(Model):
    def __init__(self, config):
        super(QuatPE, self).__init__(config)
        ...
        self.ent_transfer = nn.Embedding(self.config.relTotal, self.config.hidden_size * 4)
        self.rel_transfer_h = nn.Embedding(self.config.relTotal, self.config.hidden_size * 4)
        self.rel_transfer_t = nn.Embedding(self.config.relTotal, self.config.hidden_size * 4)
    ...
    def init_weights(self):
        if True:
            ...
            self.rel_transfer_h.data = vec2.type_as(self.rel_transfer_h.weight.data)
            self.rel_transfer_t.data = vec2.type_as(self.rel_transfer_t.weight.data)
            ...
            nn.init.xavier_uniform_(self.rel_transfer_h.weight.data)
            nn.init.xavier_uniform_(self.rel_transfer_h.weight.data)
    ...
    def forward(self):
        ...
        r_transfer_h = self.rel_transfer_h(self.batch_r)
        r_transfer_t = self.rel_transfer_t(self.batch_r)

        h1 = self._calc(h, r_transfer_h)
        t1 = self._calc(t, r_transfer_t)

        h2 = self._transfer(h, h_transfer, r)
        t2 = self._calc(t, t_transfer)

        score = torch.sum(h1 * t1, -1) + self.lam * torch.sum(h2 * t2, -1)

        regul = self.regulation(h) + self.regulation(t) + self.lam * (self.regulation(r) + self.regulation(h_transfer) + self.regulation(t_transfer)) + \
                self.regulation(r_transfer_h) + self.regulation(r_transfer_t)
