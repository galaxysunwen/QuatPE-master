...
class QuatPE3(Model):
    def __init__(self, config):
        super(QuatPE3, self).__init__(config)
    ...
    def forward(self):
        ...
        score = torch.sum(h2 * t2, -1)

        regul = self.regulation(h) + self.regulation(t) + self.regulation(r) + self.regulation(h_transfer) + self.regulation(t_transfer)
